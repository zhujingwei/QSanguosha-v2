...
public:
    ...
    void broadcastSkillInvoke(const QString &name, int type = -1, int start_index = 1) const;
    int getSkillIndex(const QString &name, int start_index = 1) const;
    ...
    bool askForSkillInvoke(const QString &skill_name, const QVariant &data = QVariant(), const bool do_notify = true);
    bool askForSkillInvoke(const Skill *skill, const QVariant &data = QVariant(), const bool do_notify = true);
    ...
    inline void setFakeHandCardNum(int num)
    {
        fakeHandCardNum = num;
    }
